...
$serviceAccountPassword = ConvertTo-SecureString "XXXXXXXXXXXX" -AsPlainText -Force
...
foreach ($sharedmailbox in $sharedmailboxes) {
    ...
    $Mailbox = Get-Mailbox -Identity $SharedMailboxName
        ...
        if ($user.Department -in $sharedmailbox.Dept.LookupValue) { #changed -eq to -in since a sharedmailbox can have multiple depts.
            ...
            if ($AllPermissions -and $AllPermissions.User -contains $user.Email) {
                #($automap -!= ){
                    ...
                    #Remove-MailboxPermission -Identity COA@RDAbbott.com -User paul.blessing@rdabbott.com -AccessRights $permissionType -Confirm:$false -WarningVariable warningOutput
                    #Write-Host"$user in $dept1 for email $SharedMailboxName"###
                #}
                #
            } else {
                Write-Host "$mail in $dept1 but not in $SharedMailboxName!!"
                $PermissionType = "FullAccess"
                Add-MailboxPermission -Identity $SharedMailbox.Email -User $user.Email -AccessRights $PermissionType -AutoMapping $automap
                Write-Host "$mail NOT in $dept1 for email $SharedMailboxName THEY WILL BE ADDED"
            }
        }
    }

    # Clear $AllPermissions after processing each shared mailbox
    $AllPermissions = @()
}
#}